...
bl_info = {
    "name" : "ICity + Smart City Generator",
    "author" : "ICity / SCG Team",
    "description" : "ICity 城市生成 + Smart City Generator 团队扩展（侧边栏 ICity 选项卡内 Smart City Generator 面板）",
    "blender" : (4, 0, 1),
    "version" : (1, 0, 4),
    "location" : "",
...
# 依据不同的城市空间类型（如 Procedural、Park、Presets、Custom），设定不同的活动属性，并将对应的值赋给网格属性。
...
# 枚举选项格式为 (identifier, name, description, icon, number)，其中：
# identifier 是选项的唯一标识符。
# name 是显示在UI中的名称。
# description 是描述（这里为空）。
# icon 是图标（这里为空）。
# number 是选项的索引。
...
def register():
    ...
    try:
        ...
        import traceback
        print("[ICity+SCG] Smart City Generator 注册失败:", exc)
        traceback.print_exc()
...
def unregister():
    try:
        ...
        import traceback
        print("[ICity+SCG] Smart City Generator 注销失败:", exc)
        traceback.print_exc()
    global _icons
